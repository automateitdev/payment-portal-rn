...
const AdmissionPreview = () => {
  ...
  } = useGetAdmissionPaymentInvoiceQuery(key, {
    skip: !key,
  });
  ...
  const {
    ...
    refetch: refetchAdmissionConfig,
    isFetching: isFetchingAdmissionConfig,
    isUninitialized: isAdmissionConfigUninitialized,
  } = useGetAdmissionDataQuery(
    instituteId ?? "",
  ...
  const {
    data: feeMap,
    refetch: refetchFeeMap,
    isFetching: isFetchingFeeMap,
    isUninitialized: isFeeMapUninitialized,
  ...
  const isRefreshing =
    isFetchingPreview ||
    isFetchingInvoice ||
    (editModalVisible && (isFetchingAdmissionConfig || isFetchingFeeMap));

  const handleRefreshAll = async () => {
    const promises: Array<Promise<any>> = [];
    if (key) {
      promises.push(refetchPreview());
      promises.push(refetchInvoice());
    }
    if (instituteId && editModalVisible) {
      if (!isAdmissionConfigUninitialized) {
        promises.push(refetchAdmissionConfig());
      }
      if (!isFeeMapUninitialized) {
        promises.push(refetchFeeMap());
      }
    }
    try {
      await Promise.allSettled(promises);
      showMessage("success", "Refreshed", "All data has been refreshed.");
    } catch {
      // ignore
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ================= PAYMENT GATEWAY WEBVIEW MODAL (MOBILE) ================= */}
        {Platform.OS !== "web" && (
          <Modal
            ...
            <SafeAreaView className="flex-1 bg-white">
              ...
              <View className="flex-row items-center justify-between px-4 py-3 border-b border-slate-200 bg-white">
                <Pressable
                  ...
                  className="flex-row items-center py-1 px-2 -ml-2 rounded-lg active:bg-slate-100"
                >
              ...
              <View className="flex-1 relative">
                {paymentUrl ? (
                  <WebView
                    ...
                    onNavigationStateChange={(navState) => {
                      ...
                      if (
                        ...
                        showMessage(
                          "success",
                          "Payment Completed",
                          "Your admission fee payment was completed successfully.",
                        );
                        handleRefreshAll();
                      } else if (
                        lower.includes("fail") ||
                    ...
                    onError={(e) => {
                      console.error("Payment WebView Error:", e);
                    }}
                  />
                ) : null}
              </View>
