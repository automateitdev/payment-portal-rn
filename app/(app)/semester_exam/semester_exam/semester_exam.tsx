...
const getTranscriptData = (
  ...
  selectedYear: string,
  selectedExam: string,
  examInfo?: { class?: string; shift?: string; section?: string; group?: string },
): TranscriptData | null => {
  if (!payload?.result?.length) {
    return null;
  }
  ...
  (studentResult.subjects || []).forEach((subject) => {
    if (subject.is_combined && subject.parts) {
      subject.parts.forEach((part) => {
        Object.keys(part.part_marks || {}).forEach((key) => markKeysSet.add(key));
      });
    } else {
      Object.keys(subject.part_marks || {}).forEach((key) =>
        markKeysSet.add(key),
  ...
  return {
    ...
    classShiftSection:
      [
        capitalize(examInfo?.class || configs.find(c => c.class_name)?.class_name || gradeItem.class_name || configItem.class_name || configItem.class),
        capitalize(examInfo?.shift || configs.find(c => c.shift_name)?.shift_name || configItem.shift_name || configItem.shift),
        capitalize(examInfo?.section || configs.find(c => c.section_name)?.section_name || configItem.section_name || configItem.section),
      ]
        .filter(Boolean)
        .join("-") || "-",
    group: capitalize(examInfo?.group || configs.find(c => c.group_name)?.group_name || configItem.group_name || configItem.group) || "-",
    merit:
      studentResult.merit_info?.class_position !== undefined
        ? `Class ${studentResult.merit_info.class_position} / Shift ${toLabelValue(
            studentResult.merit_info.shift_position,
    ...
    resultStatus: studentResult.result_status || "-",
    totalMarks: toLabelValue(studentResult.total_mark_without_optional),
    totalMarksWithOptional: toLabelValue(studentResult.total_mark_with_optional),
    totalMarksWithoutOptional: toLabelValue(
      studentResult.total_mark_without_optional,
    ),
    totalFullMarks: toLabelValue(
...
export default function SemesterExamScreen() {
  ...
  const transcriptData = useMemo(
    ...
    [resultPayload, selectedExamOption?.label, selectedYearOption?.label, selectedExamData],
  );
  ...
  const downloadPdfOnAndroid = async (fileName: string, pdfBase64: string) => {
    ...
    if (!SAF) {
      showToast("error", "Storage Access Framework is not available.");
      return;
    }
