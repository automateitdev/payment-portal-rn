...
const AdmissionEditModal = ({
    ...
    try {
      ...
      if (Platform.OS === "web" && asset.file) {
        ...
        try {
          ...
          blob =
            determinedType && rawBlob.type !== determinedType
              ? rawBlob.slice(0, rawBlob.size, determinedType)
              : rawBlob;
        } catch (e) {
          console.warn("Could not create binary blob from uri:", e);
        }
      }
  ...
  const handleUpdate = async () => {
    ...
    if (covidEdit) {
      ...
      const appendFile = async (key: string, file: FilePickAsset | null) => {
        if (!file) return;
        if (file.blob) {
          formData.append(key, file.blob, file.name);
        } else if (Platform.OS === "web" && file.webFile) {
          formData.append(key, file.webFile, file.name);
        } else if (file.uri) {
          try {
            const res = await fetch(file.uri);
            const b = await res.blob();
            formData.append(key, b, file.name);
          } catch {
            formData.append(key, {
              uri: file.uri,
              name: file.name,
      ...
      if (files.vaccine_certificate) {
        await appendFile("vaccine_certificate", files.vaccine_certificate);
      }
    }
    if (quotaEdit) {
    ...
    if (attachmentEdit) {
      ...
      for (const key of [
        ...
        await appendFile(key, files[key]);
      }
    }
